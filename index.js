...
function getArtistByIndex(anArray, index) {
    return `the artist at index ${anArray[index].id} is ${anArray[index].name}`;
}
...
function addArtist(anArray, person, lifeSpan, musicicalTaste, origin, description) {
  const newArr = [...anArray];
  const newObj = { 
    name: person, 
    years: lifeSpan,
    genre: musicicalTaste, 
    nationality: origin,
    bio: description
  } 
  newArr.push(newObj);
...
addArtist(artists, 'John Doe', '1988-2022', 'Full Stack Development', 'African American', 'I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.');
...
function lotsOfArt(anArray) {
  let newArr = []
  anArray.forEach(person => {
    if(person.paintings > 100){
...
function artistInfo(anArray, person){
  for (let i = 0; i < anArray.length; i++) {
    if(anArray[i].name === person){
       return anArray[i].bio;
     }
   };
  }
...
function artistByCountry(anArray, origin){
  const newArr = []
  anArray.map(entry => {
    if(entry.nationality === origin){
